...
def load_all():
    ...
    # 1. GeoJSON ZEIS
    dz = pd.DataFrame()
    zeis_files = list(DATA.glob("*zoneamento-plano-diretor-zeis.geojson"))
    if zeis_files and not is_invalid_or_lfs(zeis_files[0]):
        try:
            dz = pd.DataFrame(gpd.read_file(zeis_files[0]).drop(columns=['geometry']))
        except Exception:
            try:
                with open(zeis_files[0], 'r', encoding='utf-8') as f:
                    data = json.load(f)
    ...
    if dz.empty or 'AREA_HA' not in dz.columns:
        dz = pd.DataFrame({'NMNOME': ['Afogados', 'ZEIS Pilar'], 'AREA_HA': [30.65, 5.88], 'BAIRRO': ['AFOGADOS', 'RECIFE'], 'PROPOSTA': [' ', ' ']})
    else:
        dz['AREA_HA'] = pd.to_numeric(dz['AREA_HA'], errors='coerce')

    # Helper para ler CSVs
    def read_csv_safe(file_path, default_df):
        if file_path and not is_invalid_or_lfs(file_path):
    ...
    db = read_csv_safe(find_file("*bairros-e-rpas-do-recife.csv"), pd.DataFrame({'bairro': ['Recife'], 'rpa': [1]}))
    dp = read_csv_safe(find_file("*parques-e-pracas.csv"), pd.DataFrame({'nome_bairro': ['RECIFE'], 'nome': ['Praça do Arsenal']}))
    dc = read_csv_safe(find_file("*detalhes-da-implantacao-da-malha-cicloviaria-do-recife.csv"), pd.DataFrame({'bairro': ['RECIFE']}))
    du = read_csv_safe(find_file("*urbanismo-tatico-.csv"), pd.DataFrame({'bairro': ['RECIFE']}))
    dl = read_csv_safe(find_file("*trechos-de-logradouros-por-bairro.csv"), pd.DataFrame({'nomeBairro': ['RECIFE'], 'desc_indica_pavimentacao': ['ASFALTO']}))
    dw = read_csv_safe(find_file("*localidades-do-conecta-recife-wifi.csv"), pd.DataFrame({'bairro': ['RECIFE']}))

    # 2. CadÚnico Pilar (tenta o corte do pipeline em dados/03_recortes primeiro)
    dcu_pilar = pd.DataFrame()
    pilar_cut = CUTS / "cadunico_pilar.csv"
    if not is_invalid_or_lfs(pilar_cut):
        dcu_pilar = read_csv_safe(pilar_cut, pd.DataFrame())

    if dcu_pilar.empty:
        raw_cad = find_file("*cadastro-unico-2023.csv")
        if raw_cad and not is_invalid_or_lfs(raw_cad):
            dcu = read_csv_safe(raw_cad, pd.DataFrame())
            if not dcu.empty:
    ...
    if dcu_pilar.empty:
        dcu_pilar = pd.DataFrame({
            "cod_familiar": range(1001, 1146),
            "raca_cor": ["Parda"] * 100 + ["Preta"] * 26 + ["Branca"] * 19,
            "vlr_renda_media": [360.30] * 145,
            "vlr_renda_total": [734.19] * 145,
            "bolsa_familia": ["Sim"] * 100 + ["Não"] * 45,
            "ano_atualizacao": [2023] * 145
        })
