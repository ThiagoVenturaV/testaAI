...
def load_data():
    ...
    for path in required:
        if is_invalid_or_lfs(path):
            raise FileNotFoundError(f"Arquivo de dados ausente ou inválido: {path.name}. Execute `python pipeline_dados.py`.")
...
st.caption("Indicadores calculados pelo pipeline a partir das bases oficiais da Prefeitura do Recife.")
...
c2.metric("Famílias", filtered["cod_familiar"].nunique() if "cod_familiar" in filtered.columns else 0)
black_pct = filtered["raca_cor"].isin(["Preta", "Parda"]).mean() * 100 if len(filtered) and "raca_cor" in filtered.columns else 0
c3.metric("População negra", f"{black_pct:.1f}%")
families = filtered.drop_duplicates("cod_familiar") if "cod_familiar" in filtered.columns else filtered
benefit_pct = (families["bolsa_familia"] == "Sim").mean() * 100 if len(families) and "bolsa_familia" in families.columns else 0
c4.metric("Famílias no Bolsa Família", f"{benefit_pct:.1f}%")

r1, r2 = st.columns(2)
per_capita_mean = pd.to_numeric(filtered["vlr_renda_media"], errors="coerce").mean() if "vlr_renda_media" in filtered.columns else 0
family_income_mean = pd.to_numeric(families["vlr_renda_total"], errors="coerce").mean() if "vlr_renda_total" in families.columns else 0
...
lookup = indicators.set_index("indicador")["valor"].to_dict() if "indicador" in indicators.columns else {}
...
street_counts = streets["desc_indica_pavimentacao"].value_counts().rename_axis("Situação").reset_index(name="Vias") if "desc_indica_pavimentacao" in streets.columns else pd.DataFrame()
